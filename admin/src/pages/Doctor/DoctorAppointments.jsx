    ...
    <main className='min-w-0 flex-1 p-4 sm:p-6 lg:p-8'>
      <div className='mx-auto max-w-7xl'>
        <header className='mb-7 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
          <div>
            <p className='mb-1.5 text-xs font-semibold uppercase tracking-wider text-primary'>Practice management</p>
            <h1 className='text-2xl font-semibold tracking-tight text-heading sm:text-3xl'>Appointments</h1>
            <p className='mt-1 text-sm text-muted'>Review and manage your scheduled, completed, and upcoming patient consultations.</p>
          </div>
        ...
        {appointments && appointments.length > 0 ? (
              ...
              <table className='w-full border-collapse text-left text-sm'>
                <thead className='sticky top-0 z-10 border-b border-border bg-slate-50 text-xs font-semibold uppercase tracking-wider text-muted'>
                  <tr>
                    ...
                    <th scope='col' className='px-4 py-3.5 text-center'>Payment</th>
                        ...
                        {item.cancelled ? (
                          ...
                          <div className='flex items-center justify-center gap-1.5'>
                            ...
                            <button
                              ...
                              className='inline-flex h-8 w-8 items-center justify-center rounded-lg transition-colors hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2'
                              aria-label={`Mark appointment completed for ${item.userData.name}`}
                              title='Complete appointment'
                            >
                              <img src={assets.tick_icon} className='h-5 w-5 object-contain' alt='' />
                            </button>
                          </div>
                        )}
                      </td>
                      ...
                      {item.cancelled ? (
                        ...
                        <div className='flex items-center gap-1'>
                          ...
                          <button
                            type='button'
                            onClick={() => completeAppointment(item._id)}
                            className='inline-flex h-8 w-8 items-center justify-center rounded-lg transition-colors hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary'
          ...
          <div className='flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface p-12 text-center'>
            <div className='flex h-12 w-12 items-center justify-center rounded-full bg-primary-light text-primary'>
              <img className='h-6 w-6 object-contain opacity-70' src={assets.appointment_icon} alt='' />
            </div>
            <h3 className='mt-4 text-base font-semibold text-heading'>No appointments found</h3>
            <p className='mt-1 max-w-sm text-sm text-muted'>
              There are currently no patient appointments booked for your schedule.
            </p>
          </div>
        )}
      </div>
